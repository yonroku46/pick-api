...
public class TalkController {
    ...
    @GetMapping("/roomList")
    public SingleResponse<ResponseData> talkRoomList(TalkRoomListReqDto talkRoomListReqDto) {
    ...
    @GetMapping("/enter")
    public SingleResponse<ResponseData> enterTalkRoom(EnterTalkRoomReqDto enterTalkRoomReqDto) throws IllegalAccessException {
        return responseService.getSingleResponse(talkService.findMessages(enterTalkRoomReqDto));
    }

    // 채팅방 새로 생성 -> talkRoomCd 리턴 (-> redirect하기)
    @GetMapping("/create")
    public SingleResponse<ResponseData> createTalkRoom(CreateTalkReqDto createTalkReqDto) {
